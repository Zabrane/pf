...
//! strlen|negate|parseunsigned|longtostr|atoi
...
ZI txp(S x,I n,I p){R txN(' ',MX(0,p-n))+txn(x,n)+txN(' ',ABS(MN(0,p+n)));}ZI txb(G c,I p){R txp(&c,1,p);}
I txx(J j,I p){I n=jX(j)+2;S b=xb+25-n;*b='0',b[1]='x';R txp(b,n,p);}
I txj(J x,I p){I n;S r=jS(x,&n);R txp(r,n,p);}I txs(S x,I p){R txp((S)x,sln(x),p);}
...
I main(I c,char**a){
  ...
  pf(" n=(%d) %s\n\n",
   pf("\n pf: s=(%s %s %s) p=(0x%p) c=(%c) eot=(0x%p)",
